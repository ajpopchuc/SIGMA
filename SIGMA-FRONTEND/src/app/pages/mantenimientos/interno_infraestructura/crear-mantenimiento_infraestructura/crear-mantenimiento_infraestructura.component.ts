...
import { MantenimientoService } from '../mantenimiento_infraestructura.service';
...
@Component({
  selector: 'app-crear-mantenimiento_infraestructura', 
  standalone: true,
  templateUrl: './crear-mantenimiento_infraestructura.component.html',
  styleUrls: ['./crear-mantenimiento_infraestructura.component.css'],
  imports: [ReactiveFormsModule, CommonModule]
})
export class CrearMantenimientoInfraestructuraComponent  {
  ...
  constructor(
    ...
    this.mantenimientoForm = this.fb.group({
      ...
      lugar_mantenimiento: ['Infraestructura'],
      observaciones: [''],
      ...
      detalle_mantenimiento: this.fb.array([this.crearDetalleMantenimiento()]),
      area:['']  // Add this line
    })
  ...
  private initWizard() {
    ...
    this._wizardObj.on('change', (wizard: any) => {
      ...
      if (step === 1) {
        ...
        //Verificar que el campo de fotografía inicial array no esté vacío
      ...
      if (step === 2) {
        ...
        if (!this.mantenimientoForm.value.id_activos) {
          ...
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Por favor, ingresa un elemento'
          });
          return;
        }
        ...
        (response) => {
          ...
          if(response.status === 200) {
            Swal.fire({
              ...
              text: 'Mantenimiento de infraestructura creado correctamente'
            });
            sessionStorage.removeItem('data_recolectada');
            this.router.navigate(['/mantenimientos/infraestructura']);
          }
          else {
            Swal.fire({
              icon: 'error',
  ...
  getRecursos(): void {
    this.mantenimientoService.getRecursos().subscribe(
      (data) => {
        this.recursos = data;
        this.filteredRecursos = [...this.recursos]; // Inicializar la lista filtrada
  ...
  // Filtra los recursos según el área seleccionada
