...
import { SupervisionService } from '../supervision_infraestructura.service';
...
@Component({
  selector: 'app-crear-supervision_infraestructura',
  templateUrl: './crear-supervision_infraestructura.component.html',
  styleUrls: ['./crear-supervision_infraestructura.component.css'],
  standalone: true,
  imports: [ReactiveFormsModule, CommonModule, ZXingScannerModule]

})
export class CrearSupervisionInfraestructuraComponent implements AfterViewInit, OnInit {
  ...
  constructor(
    ...
    this.supervisionForm = this.fb.group({
      id_mantenimiento: ['1', Validators.required],
      ...
      lugar_mantenimiento: ['Infraestructura'], //esta parte me faltaba creo para poner guardar bien el lugar de mantenimiento
      // paso 3
  ...
  ngOnInit(): void {
    ...
    const idMantenimiento = this.supervisionForm.get('id_mantenimiento'); // el id_mantenimiento es el tipo de mantenimiento pero necesito solo el id
  ...
  private initWizard() {
    ...
    this._wizardObj.on('change', (wizard: any) => {
      ...
      if (step === 4) {
        ...
        //Guardo el tiempo de ejecución en una variable para poder mostrarlo en el resumen en minutos con dos decimales
          ...
          (response) => {
            ...
            if(response.status === 200) {
              Swal.fire({
                ...
                text: 'Supervision de infraestructura creada correctamente'
              });
              this.router.navigate(['/supervisiones/infraestructura']);
            }
            else {
              Swal.fire({
                icon: 'error',
