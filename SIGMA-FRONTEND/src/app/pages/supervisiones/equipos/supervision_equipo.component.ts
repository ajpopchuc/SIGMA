...
import { SupervisionService, Usuario, Actividad } from './supervision_equipo.service';
...
@Component({
  selector: 'app-supervision_equipo',
  templateUrl: './supervision_equipo.component.html',
  styleUrls: ['./supervision_equipo.component.css'],
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, NgxPaginationModule, ReactiveFormsModule]
})
export class SupervisionEquipoComponent implements OnInit {
  ...
  tipos_actividad = ['Mantenimiento a equipo' ]; // Ajusta según tus tipos
  ...
  ngOnInit(): void {
    ...
    console.log('Supervisiones:', this.supervisiones);
  }

  fetchSupervisiones(): void {
    this.supervisionService.getSupervision().subscribe(
      (data) => {
        console.log('Fetched supervisiones:', data);
        this.supervisiones = data.data;
        this.filteredSupervisiones = this.supervisiones; // Clona el array
        console.log('Supervisiones después de asignación:', this.supervisiones); // Revisa el array
