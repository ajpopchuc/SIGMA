...
export class EditarEmpleadoComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.empleadoId = this.empleado?.id || 0;
  }
          ...
          (response) => {
            if (response.status === 200) {
              ...
              Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Ha ocurrido un error al modificar el empleado: ' + response.message
              });
            }
          },
          (error) => {
  ...
  getRolNombre(): string {
    const rolId = this.empleadoForm.value.rol;
